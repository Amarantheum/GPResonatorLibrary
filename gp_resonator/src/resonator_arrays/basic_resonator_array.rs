...
/// See [`BasicResonatorArray::process_buf`].
#[derive(Debug, Clone)]
pub struct BasicResonatorArray {
    /// The sample rate of audio being passed through the resonator
    pub sample_rate: f64,
    resonators: Vec<(ConjPoleResonatorState, ConjPoleResonator)>
}

impl BasicResonatorArray {
    /// Creates a new instance of [`BasicResonatorArray`] with given sample rate
    ...
    /// Returns the number of resonators in the [`BasicResonatorArray`]
    #[inline]
    ...
    /// # Arguments
    /// * `f` - A function the operates on each [`ConjPoleResonator`] in the [`BasicResonatorArray`]
    #[inline]
...
impl BodePlotTransferFunction for BasicResonatorArray {
...
mod tests {
    ...
    fn test_add_resonator() {
        let mut array = BasicResonatorArray::new(48_000.0, 1);
